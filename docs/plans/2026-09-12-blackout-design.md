...
## Memory: the void and topic stats

Decided 2026-09-13. Every answer is an append-only log entry on the device,
mirrored to Supabase when signed in with Google (see `docs/hosting.md`).
Derived from the log: the void (questions missed and not yet released),
each question's streak, what has been seen, and per-topic accuracy.

- A miss puts a question in the void. Three correct in a row, across runs
  and wherever answered, release it. A miss resets the streak.
- A run opens with a void session: up to ten void questions, random, no
  repeats, none if the void is empty. Grey band, lights hidden, no lights
  lost. Then the map.
- The title screen says how many questions the void holds. Map labels show
...
- `supabase/schema.sql`: the attempts table and its row-level security.
- `docs/hosting.md`: hosting decisions and the one-time Supabase setup.
